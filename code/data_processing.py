...
def prepare_data(path, one_hot = True, binary = True, normalize = True):
    ...
    If one_hot is True, columns with more than 2 categories are one hot
    endcoded. If binary is True, columns with 2 categories are replaced with
    binary numbers.
    """
    ...
    # Drop the 'other' sample for gender because there is only one sample
    data = data[data.gender != 'Other']

    # create columns with binary values if binary is true
    if binary:
    ...
    if normalize:
        ...
        # create a list with column names of numerical data
        numeric_data = ['age', 'bmi', 'avg_glucose_level']

        # normalize the numeric values using the zscore and replace these
